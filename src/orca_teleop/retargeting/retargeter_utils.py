...
def extract_mano_fingertips_and_palm(
    joints: torch.Tensor, fingers: list[str], source: str
    ...
    mano_palm = torch.mean(
        torch.cat([mano_bases["thumb"], mano_bases["pinky"]], dim=0), dim=0, keepdim=True
...
def extract_orca_fingertips_and_palm(
    ...
    orca_thumb_base = chain_transforms[
        get_finger_base_urdf_name(hand_type, "thumb")
    ].transform_points(root)
    orca_pinky_base = chain_transforms[
        get_finger_base_urdf_name(hand_type, "pinky")
    ].transform_points(root)
    orca_palm = torch.mean(
        torch.cat([orca_thumb_base, orca_pinky_base], dim=0), dim=0, keepdim=True
    )
    palm_offset = torch.tensor(
        [0, 0, 0.015], device=orca_palm.device
    )  # Hardcoded offset to align the keypoints better
...
def get_canonical_key_vectors(
    joint_pos: np.ndarray, source: str, fingers: list[str] | None = None
) -> np.ndarray:
    """Build fixed-order palm-to-fingertip key vectors from source-specific joint positions."""

    if fingers is None:
        fingers = ["thumb", "index", "middle", "ring", "pinky"]

    normalized_joint_pos = get_normalized_local_manohand_joint_pos(joint_pos, source)
    joint_tensor = torch.tensor(normalized_joint_pos, dtype=torch.float32)
    fingertips, palm = extract_mano_fingertips_and_palm(joint_tensor, fingers, source)
    return torch.cat(get_keyvectors(fingertips, palm), dim=0).cpu().numpy()


def rotate_points_around_y(
    joints: np.ndarray, angle_degrees: float, source: str, hand_type: str = "right"
) -> np.ndarray:
...
def get_urdf_model_params(
    chain, hand_type: str, fingers: list[str], root_point: torch.Tensor
    ...
    urdfhand_center, urdfhand_rot_matrix = get_hand_center_and_rotation(
